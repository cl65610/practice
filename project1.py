...
import numpy as np
#%matplotlib inline
#Creating lists for quicker references
rates = [x for x[1] in data]
maths = [x for x[3] in data]
verbal =[x for x[2] in data]
#plotting Histogram for SAT rate
plt.hist(rates)
plt.title('SAT Rate')
...
plt.axis([0, 1, 0, 20])
plt.show()
#Histogram for Reading Distribution
plt.hist([x[2] for x in data])
