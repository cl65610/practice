...
dist_mean = mean(dist)
print 'The mean of the data set is %.2f.' % dist_mean

dist_median = median(dist)
print 'The median of the data set is %.2f.' % dist_median

dist_mode = mode(dist)
print 'The mode of the data set is %.2f.' % dist_mode.mode[0]


'Skewness: The mean of this data set is larger than the median, so it has a positive skew to the right.'

dist_var = np.var(dist)
print 'The variance of the dataset is %.2f' % dist_var

dist_dev = np.std(dist)
print 'The standard deviation of the dataset is %.2f' % dist_dev

'''The standard deviation for a data set tells us how how far, on average, numbers are from the mean.
It's a nice way to get a sense of how tightly-packed around the mean the data set is. The closer to zero
the standard deviation is, the more-tightly packed the data.'''
uniques = set(dist)
instances ={}

for i in uniques:
      instances[i] =dist.count(i)
max = 0
...
for i in instances:
    if instances[i] == max:
        print 'The mode of the dataset it %d. It occurs %d times.' % (i, max)
